  ...
  useEffect(() => {
    ...
    fetchTenDayForecast(telemetry.gps.latitude, telemetry.gps.longitude).then((data) => {
      if (active) {
        ...
        // Update real-time instantaneous solar generation (Watts) based on actual current hour solar radiation
        const currentHour = new Date().getHours();
        const currentHourForecast = data.daily[0]?.hourlyData[currentHour] || data.hourly[currentHour];
        if (currentHourForecast) {
          const currentRadWatts = currentHourForecast.solarRadiationWatts || 0;
          // Calculate solar power generated by cold-storage 1.2 kWp array: 1200 * (W/m² / 1000) * 0.82
          const liveSolarWatts = Math.round(1200 * (currentRadWatts / 1000) * 0.82);
          setTelemetry((prev) => ({
            ...prev,
            solarPowerWatts: Math.max(0, liveSolarWatts),
          }));
        }
      }
    });
